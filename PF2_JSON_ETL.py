...
for i in range(0,totalpoke):
    ...
    if ("-" in pokemon.iloc[i,14] and pokemon.iloc[i,14] not in img_url_cap_exception) or pokemon.iloc[i,14] in img_url_exception:
        pokemon.iloc[i,16] = f"static/images/PokePics/{pokemon.iloc[i,14]}.gif"
    else:
        pokemon.iloc[i,16] = f"static/images/PokePics/{name_cap}.gif"
    if pokemon.iloc[i,14] == "ho-oh":
        pokemon.iloc[i,16] = "static/images/PokePics/Ho-Oh.gif"
    if pokemon.iloc[i,14] == "porygon-z":
        pokemon.iloc[i,16] = "static/images/PokePics/Porygon-Z.gif"
    #pokemon.iloc[i,16] = f"static/images/PokePics/{pokemon.iloc[i,14]}.gif"
    # **************************************************************************************************
...
### Combat Variables ---------------------------------
# Read Data
type_matchups = pd.read_csv("data/type_matchups.csv").set_index("TYPE")
# Create Variables
type_coeffs = {}
wip_dict_type = {}
types = []
...
pokedex = json.dumps(pokedex_dict)
combat_vars = json.dumps(combat_vars_dict)
match_vars = json.dumps(match_vars_dict)

# Saving JSON objects to local files
with open("pokedex_data.json", "w") as outfile:
    outfile.write(pokedex)
with open("combat_vars.json", "w") as outfile:
    outfile.write(combat_vars)
with open("match_vars.json", "w") as outfile:
    outfile.write(match_vars)
